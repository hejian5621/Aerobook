...
class  test_UseCaseSet_Aerocheck(unittest.TestCase):
    ...
    def __init__(self, *args):
        ...
        self.Use_attribute =None


    """ 执行测试用例步骤 """
    @data(*list_dict_site)  # 参数化参数用例，主要的用于用例的循环测试，并且把测试点放入测试报告
    def test_1(self, testCase):
        self.actual_result = UseCase_step(self.Use_attribute, self.dict_testCase).Perform_useCase_Steps()
    ...
    def setUp(self):
        ...
        print("")
        """ 用例执行前具体的准备工作"""
        dictSet = {"全局用例集名称": UseSet_n,"控件属性已经操作方法": self.Use_attribute,"全局主模块名称":host_Module,"用例运行总次数":useCase_sum}
        pack_dict = Initializing().controller(dictSet,self.dict_testCase)
        # 取出参数
        UseSet_n=pack_dict["全局子模块名称"]
        host_Module=pack_dict["全局主模块名称"]
        self.Use_attribute=pack_dict["控件属性已经操作方法"]

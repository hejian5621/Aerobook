...
class Test_Laminatedata(unittest.TestCase):
        ...
        def test_1(self, testdicts):
            ...
            actual_result,actual_editlist = Laminatedata_execute().SelectFile(testdicts)  # 调用测试步骤
            # 断言测试结果
...
class Test_sizeInfo_1DXls(unittest.TestCase):
        ...
        def setUp(self):
            ...
            old_content = ActualProcessing(None).acquire_HTML_TXT(source)
...
class Test_solveCalculation(unittest.TestCase):
        ...
        def setUp(self):
            ...
            instrument().delfile(source,list_filePath)
            # 用例执行之前清楚项目文件夹里的部分文件

...
class  fastener_parameterInput(unittest.TestCase):
    """紧固件强度校核--紧固件参数输入"""
    ...
    def setUp(self):
        ...
        real_UseCase_Name = ProfileDataProcessing("TestCase_moduleName",
                                                  "UseCaseName_12").config_File()  # 执行该用例模块的标识
    ...
    # Name=["其他"]
    Name = ["测试"]
    real_UseCase_Name = ProfileDataProcessing("TestCase_moduleName","UseCaseName_12").config_File()  # 执行该用例模块的标识
...
@ddt
# @unittest.skip(u"暂时不执行")
class fastener_parameterSetting(unittest.TestCase):
    """紧固件强度校核--紧固件参数设置"""
    global real_UseCase_Name
    ...
    def setUp(self):
        ...
        # 用例执行前，初始化测试结果相关的文件
        real_UseCase_Name = ProfileDataProcessing("TestCase_moduleName",
                                                  "UseCaseName_13").config_File()  # 执行该用例模块的标识
        dictSet = {"全局参数": number, "全局用例集名称": global_UseCase_Name, "当前用例集名称": real_UseCase_Name,
                   "控件属性已经操作方法": testCase_attribute}
        number, global_UseCase_Name, ProjectPath, old_content, testCase_attribute = Initializing(). \
            controller(dictSet)  # 铺层信息--铺层库优化工作栏测试用例
    ...
    def test_1(self, testCase_dict):
        ...
        """测试用例步骤"""
        actual_result = UseCase_step(testCase_attribute, testCase_dict). \
            Perform_useCase_Steps()  # 紧固件强度校核--紧固件参数设置


if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTests(unittest.TestLoader().loadTestsFromTestCase(fastener_parameterInput))
    suite.addTests(unittest.TestLoader().loadTestsFromTestCase(fastener_parameterSetting))
